...
├── сокращения.txt              ← справочник сокращений для нормализации
├── raw/                        ← сюда поместить исходные CSV ОЖФ
├── geopackage/
...
Перед первым запуском поместить исходные CSV-файлы ОЖФ в папку `raw/`. 
Файлы должны быть в формате pipe-delimited (`|`), кодировка UTF-8.
Скрипт автоматически найдёт все `*.csv` в этой папке.
...
**Вход:** CSV-файлы в `raw/` (pipe-delimited, кодировка UTF-8)
...
1. Добавить новые CSV-файлы ОЖФ в `raw/` (автоматически подхватятся)
